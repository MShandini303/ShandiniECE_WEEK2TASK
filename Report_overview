...
According to the surrounding content in your Week 2 task, the goal is to build a foundational understanding of SoC architecture and practice functional modeling using tools like Icarus Verilog and GTKWave.
   ...
   ###In short, BabySoC is like a “training bicycle” for SoC design—it’s not just simplified, it’s strategically minimal to help learners build intuition, confidence, and symbolic clarity before tackling full-scale chip design
      ...
      >>>Hands-On Simulation: It allows learners to simulate system behavior using open-source tools, helping them visualize how data flows between modules before diving into RTL or physical design.

      >>>Conceptual Clarity: BabySoC acts as a bridge between theory and practice. It’s small enough to understand in full, yet rich enough to demonstrate real SoC principles.

      >>>Functional Modeling Focus: The model supports early-stage functional modeling, which is critical for verifying logic and system behavior before committing to RTL synthesis or layout. This reduces errors and improves design confidence.
